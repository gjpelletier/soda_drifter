...
% - - - - -
% Trajectories of passive drifters using monthly velocity u and v from SODA3.12.2 using Euler's method
...
soda.ver = 'v56';					% version number for output file names
...
soda.pickstartyear = 2015;			% the year when the drifter experiment starts			
soda.pickstartmonth = 6;			% the month to start the drifter experiment in the start year (e.g. 1=Jan, 2=Feb, etc)
soda.pickendyear = 2016;  			% the year when the drifter experiment ends. for one year simulation input the end year with same value as the start year 
soda.pickendmonth = 6;				% the month to end the drifter experiment in the end year (e.g. 12=Dec, 11=Nov, etc)
...
soda.drifter_width = .5;			% east-west width of each drifter node (degrees)
soda.drifter_height = .5;			% north-south height of each drifter node (degrees)
...
if strcmp(soda.whichdrifters,'Spot')	% User-specified spot location
	...
	if soda.forward_tracking
		...
		soda.d01.lon(1,:) = soda.drifter_spot_lon;					% degE
		soda.d01.lat(1,:) = soda.drifter_spot_lat;					% degN
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,:) = soda.drifter_spot_lon;	% degE
		soda.d01.lat(soda.picknmonths,:) = soda.drifter_spot_lat;	% degN
	end
elseif strcmp(soda.whichdrifters,'Feely') 	% offshore stations at 2007 West Coast cruise stations in Fig 1 by Feely,R.A.,etal.,ChemicalandbiologicalimpactsofoceanacidiﬁcationalongthewestcoastofNorthAmerica, Estuarine, Coastal and Shelf Science (2016), http://dx.doi.org/10.1016/j.ecss.2016.08.043
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,:) = [227.135	230.517	232.456	233.069	232.771	233.566	234.892	236.135	237.412	238.953	240.478	243.412	245.218];		% degE
		soda.d01.lat(1,:) = [50.669	48.277	46.034	44.606	41.383	39.29	37.662	35.884	34.173	32.578	30.319	27.196	25.053];			% degN
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,:) = [227.135	230.517	232.456	233.069	232.771	233.566	234.892	236.135	237.412	238.953	240.478	243.412	245.218];	% degE
		soda.d01.lat(soda.picknmonths,:) = [50.669	48.277	46.034	44.606	41.383	39.29	37.662	35.884	34.173	32.578	30.319	27.196	25.053];	% degN
	end
elseif strcmp(soda.whichdrifters,'pteropods') 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lat(1,1:36) = 50;		% degN
		soda.d01.lon(1,1) = 360-135;	% degE
		for i = 2:36
		...
		% final position of drifters
		soda.d01.lat(soda.picknmonths,1:36) = 50;	% degN
		soda.d01.lon(soda.picknmonths,1) = 360-135;	% degE
		for i = 2:36
...
elseif strcmp(soda.whichdrifters,'153W') 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,1:soda.ndrift) = 360-153;	% degE
		% soda.d01.lat(1,1) = 58;
		soda.d01.lat(1,1) = 57;						% degN
		for i = 2:soda.ndrift
			soda.d01.lat(1,i) = soda.d01.lat(1,i-1) - 0.5;
		end
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,1:soda.ndrift) = 360-153;		% degE
		% soda.d01.lat(soda.picknmonths,1) = 58;
		soda.d01.lat(soda.picknmonths,1) = 57;						% degN
		for i = 2:soda.ndrift
			soda.d01.lat(soda.picknmonths,i) = soda.d01.lat(soda.picknmonths,i-1) - 0.5;
		end
	end
elseif strcmp(soda.whichdrifters,'135W') 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,1:soda.ndrift) = 360-135;		% degE
		soda.d01.lat(1,1) = 55;							% degN
		for i = 2:soda.ndrift
			soda.d01.lat(1,i) = soda.d01.lat(1,i-1) - 0.5;
		end
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,1:soda.ndrift) = 360-135;		% degE
		% soda.d01.lat(soda.picknmonths,1) = 58;
		soda.d01.lat(soda.picknmonths,1) = 55;						% degN
		for i = 2:soda.ndrift
			soda.d01.lat(soda.picknmonths,i) = soda.d01.lat(soda.picknmonths,i-1) - 0.5;
		end
	end
elseif strcmp(soda.whichdrifters,'130W') 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,1:soda.ndrift) = 360-130;		% degE
		soda.d01.lat(1,1) = 52;							% degN
		for i = 2:soda.ndrift
			soda.d01.lat(1,i) = soda.d01.lat(1,i-1) - 0.5;
		end
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,1:soda.ndrift) = 360-130;		% degE
		% soda.d01.lat(soda.picknmonths,1) = 58;
		soda.d01.lat(soda.picknmonths,1) = 52;						% degN
		for i = 2:soda.ndrift
			soda.d01.lat(soda.picknmonths,i) = soda.d01.lat(soda.picknmonths,i-1) - 0.5;
		end
	end
elseif strcmp(soda.whichdrifters,'33N') 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lat(1,1:soda.ndrift) = 33;			% degN
		soda.d01.lon(1,1) = 360-118;				% degE
		for i = 2:soda.ndrift
			soda.d01.lon(1,i) = soda.d01.lon(1,i-1) - 0.5;
		end
	else		% reverse drifter tracking
...
elseif strcmp(soda.whichdrifters,'50N') 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lat(1,1:soda.ndrift) = 50;		% degN
		soda.d01.lon(1,1) = 360-128;			% degE
		for i = 2:soda.ndrift
			soda.d01.lon(1,i) = soda.d01.lon(1,i-1) - 0.5;
		end
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lat(soda.picknmonths,1:soda.ndrift) = 50;		% degN
		soda.d01.lon(soda.picknmonths,1) = 360-128;				% degE
		for i = 2:soda.ndrift
			soda.d01.lon(soda.picknmonths,i) = soda.d01.lon(soda.picknmonths,i-1) - 0.5;
		end
	end
elseif strcmp(soda.whichdrifters,'Japan') 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,1:51) = 160;		% degE
		soda.d01.lat(1,1) = 50;			% degN
		for i = 2:51
			soda.d01.lat(1,i) = soda.d01.lat(1,i-1) - 0.5;
		end
		soda.d01.lat(1,52:96) = 25;		% degN
		soda.d01.lon(1,52) = 159.5;		% degE
		for i = 53:96
			soda.d01.lon(1,i) = soda.d01.lon(1,i-1) - 0.5;
		end
	else		% reverse drifter tracking
		soda.d01.lon(soda.picknmonths,1:51) = 160;		% degE
		soda.d01.lat(soda.picknmonths,1) = 50;			% degN
		for i = 2:51
			soda.d01.lat(soda.picknmonths,i) = soda.d01.lat(soda.picknmonths,i-1) - 0.5;
		end
		soda.d01.lat(soda.picknmonths,52:96) = 25;		% degN
		soda.d01.lon(soda.picknmonths,52) = 159.5;		% degE
		for i = 53:96
			soda.d01.lon(soda.picknmonths,i) = soda.d01.lon(soda.picknmonths,i-1) - 0.5;
		end
	end
elseif strcmp(soda.whichdrifters,'Kuroshio')	% Kuroshio Current transport transect off the SW coast of Japan
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,:) = [141	141.375	141.75	142.125	142.5 142.875 143.25];		% degE
		soda.d01.lat(1,:) = [36	35.5	35	34.5	34 33.5 33];						% degN
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,:) = [141	141.375	141.75	142.125	142.5 142.875 143.25];	% degE
		soda.d01.lat(soda.picknmonths,:) = [36	35.5	35	34.5	34 33.5 33];				% degN
	end
elseif strcmp(soda.whichdrifters,'Oyashio') 	% Oyashio current transport transect at East Kamchatka
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,:) = [160	160.625	161.25	161.875	162.5	163.125];		% degE
		soda.d01.lat(1,:) = [54	53.6875	53.375	53.0625	52.75	52.4375];			% degN
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,:) = [160	160.625	161.25	161.875	162.5	163.125];	% degE
		soda.d01.lat(soda.picknmonths,:) = [54	53.6875	53.375	53.0625	52.75	52.4375];	% degN
	end
elseif strcmp(soda.whichdrifters,'KE')		% Kuroshio Extension transport transect 
	...
	if soda.forward_tracking
		% initial position of drifters
		soda.d01.lon(1,:) = 160;			% degE
		soda.d01.lat(1,:) = [49:-0.5:28];	% degN
	else		% reverse drifter tracking
		% final position of drifters
		soda.d01.lon(soda.picknmonths,:) = 160;				% degE
		soda.d01.lat(soda.picknmonths,:) = [49:-0.5:28];	% degN
	end
end
...
% soda meshgrid x and y arrays corresponding to the input u and v arrays, used by interp2 and m_map quiver plots
